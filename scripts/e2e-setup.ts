...
// issue 437: rovnaký mechanizmus a dôvod ako `E2E_ULOHY_EMAIL` vyššie — nový
// spec súbor (`poznamky.spec.ts` — "Poznámky", zdieľaná nástenka + PWA)
// dostáva VLASTNÝ izolovaný účet. Rola "sef" ZÁMERNE — zrkadlí reálneho
// používateľa (Štěpán), a test tým dokazuje, že aj rola `sef` môže na
// zdieľanej nástenke pridávať/vybaviť/mazať (žiadny role-gating,
// `note-routes.ts`).
...
// issue 443: DRUHÁ nedostupná objednávka toho ISTÉHO variantu "40287" — aby mala
// skupina na "Nedostupné tovary" DVE objednávky a hlavička ukázala súčtový odznak
// "Σ 2" (`nedostupne.spec.ts`). Pridáva GLOBÁLNE ešte 1 riadok do "Na objednanie"
// bez dodávateľa (vzor issue 62/176: `orders.spec.ts` počíta so "Všetci (9)"/
...
// na prvú objednávku).
const [objednavkaNedostupne2] = await db.insert(orders).values({ externalOrderId: "9009", customerName: "E2E Druhá Objednávka", statusName: DEFAULT_ORDER_OPEN_STATUS, placedAt: new Date("2026-07-28T09:00:00Z"), email: "e2e-nedostupne@forestshop.sk" }).returning();
