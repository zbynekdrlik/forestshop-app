...
- **Tajomstvá nikdy v gite.** `SHOPTET_EXPORT_URL` obsahuje prihlasovací `hash` v query parametri — je len v `.env` na dev2 a v GitHub Secrets. Do databázy ani do logov sa nikdy nezapisuje celá URL, len jej verzia s prekrytým `hash` (`redactUrl`). Fixtúra vyrezaná z reálneho exportu obsahuje **iba produktové dáta** (žiadne osobné údaje, žiadny prihlasovací údaj), preto sa smie commitnúť — **s podmienkou (review final-wave-a, 2026-07-29): stĺpec `supplier` a všetky štyri peňažné stĺpce (`price`, `standardPrice`, `purchasePrice`, `actionPrice`) musia byť SYNTETICKÉ, nikdy reálne hodnoty z produkčného exportu.** Repo je verejné — reálne dodávateľské mená vedľa reálnych nákupných a predajných cien by boli maržou tohto obchodu, verejne čitateľnou kýmkoľvek. Tvar hodnôt (dve desatinné miesta, desatinná čiarka, prázdne bunky tam, kde boli prázdne) zostáva zachovaný, len čísla a mená sú vymyslené.
...
- Produces:
  ...
  - `splitCode(code: string): { readonly productKey: string; readonly sizeLabel: string | null }` (review task-5-fix-1, CRITICAL #1: vrátený `productKey`, teda prefix `code` pred prvou lomkou, UŽ NIE JE identita produktu — na reálnom exporte sa v oboch smeroch mýlil, zlučoval nesúvisiace produkty pod rovnaký prefix aj rozdeľoval jeden produkt naprieč viacerými prefixmi. Funkcia teraz slúži len na odvodenie `sizeLabel` a na detekciu kódu začínajúceho lomkou. Identita produktu je export's `guid`.)
  - `interface VariantRecord` (presné polia nižšie — Task 5 ich vkladá 1:1 do tabuľky `variant`; review task-5-fix-1 pridal pole `readonly guid: string` a zmenil `productKey`, aby niesol `guid` z riadku, nie prefix `code`)
  - `interface RowIssue { readonly kind: IngestIssueKind; readonly code: string; readonly detail: Record<string, string> }`
  - `type IngestIssueKind = "empty_code" | "empty_guid" | "duplicate_code" | "invalid_money" | "missing_currency" | "invalid_stock" | "product_name_conflict"` (review task-5-fix-1 pridal `"empty_guid"` — prázdny `guid` sa zaobchádza rovnako ako prázdny `code`: žiadny fallback, žiadny záznam)
...
**CRITICAL — identita produktu je `guid`, nie prefix `code` (review task-5-fix-1, doplnené po prvej implementácii Task 4/5):** Merané proti reálnemu 54 MB exportu, pravidlo „prefix `code` pred prvou lomkou" bolo chybné v oboch smeroch — prefixové zoskupovanie zlúčilo 282 úplne nesúvisiacich produktov pod junk-prefix `"997"` a súčasne ticho rozdelilo 71 skutočných produktov naprieč viacerými prefixmi (napr. `B13/S` a `B23/S`, ten istý `guid`, iný prefix). Export's `guid` je stabilný identifikátor zo Shoptetu, jeden na produkt, na každom riadku, a nikdy nemá dva rôzne `pairCode` (overené na reálnom exporte: 0 takých prípadov, 4519 distinct guidov = presne toľko, koľko má mať katalóg). Preto:
- `product.key` = `guid` (hodnota, nie schéma — stĺpec `key` ostáva `text` primary key).
- `variant` dostáva NOVÝ stĺpec `guid: text NOT NULL` (migrácia `apps/api/drizzle/0003_*.sql`) — uložený PRIAMO na riadku, nezávisle od FK `product_key`, presne z rovnakého dôvodu ako gzipnuté surové bajty v `raw-store.ts`: surový identifikátor zo zdroja sa nedá spätne zrekonštruovať, ak by sa niekedy zmenil model zoskupovania produktov.
- `guid` pribudol do `REQUIRED_COLUMNS` (`validation.ts`) — export bez neho sa odmietne rovnako ako bez `supplier` (#281).
- `pairCode` zostáva len pozorovaná vlastnosť (nikdy kľúč) a `code`-prefix si ponecháva jedinú svoju zvyšnú robotu: odvodenie `sizeLabel`.
- Skutočné čísla po prepnutí na reálnom exporte: **14 014 variantov, 4 519 produktov, 0 anomálií `product_name_conflict`** (predtým 4 238 produktov a 581 anomálií, z toho 281 spôsobených výhradne chybným zoskupovaním).
...
> **Prekonané — pozri CRITICAL dodatok vyššie.** Blok nižšie ešte testuje starý kľúč
> (`productKey` = prefix `code` pred prvou lomkou); identita produktu je `guid`.
...
> **Prekonané — pozri CRITICAL dodatok vyššie.** `mapRow` nižšie odvodzuje
> `productKey` len cez `splitCode(code)` a nikde nečíta `row["guid"]`, ani
> nepridáva `empty_guid`; identita produktu je `guid`, nie tento prefix.
...
- Produces:
  ...
  - `INGEST_BATCH_SIZE = 500` (review task-5-fix-1: tabuľka `variant` má po pridaní `guid` 25 stĺpcov, nie 21 — 500 riadkov na dávku dá ~12 500 parametrov, stále bezpečne pod limitom 65 535)
  ...
  - `type CatalogIngestResult = { status: "accepted"; snapshotId: string; variantCount: number; productCount: number; missingCount: number; issueCount: number } | { status: "rejected"; snapshotId: string; reason: string } | { status: "duplicate"; snapshotId: string }` (`missingCount` = NOVO označené ako chýbajúce PRI TOMTO importe, nie celkový počet chýbajúcich v katalógu — review task-5-fix-1)
  ...
  - `redactSourceLabel(label: string): string` (review task-5-fix-1 — bezpečná obálka nad `redactUrl` pre volanie zo SLUŽBY: `ingestCatalog` dôveruje `sourceLabel` od AKÉHOKOĽVEK vstreknutého fetchera, nielen `createHttpExportFetcher`; nie-URL popisok necháva nezmenený namiesto vyhodenia)
  - env polia `SHOPTET_EXPORT_URL?: string`, `CATALOG_RAW_DIR: string`

**Review task-5-fix-1 (CHANGES REQUESTED, dve CRITICAL + tri Important + šesť Minor nálezov) — zhrnutie zmien oproti pôvodnému Step-by-Step nižšie** (samotné historické kroky nižšie zostávajú nedotknuté, toto zhrnutie je záväzný dodatok):

- **CRITICAL — identita produktu je `guid`** (`product.key`, aj nový stĺpec `variant.guid`) — pozri detailné zdôvodnenie a namerané čísla v CRITICAL boxe pri Task 4 vyššie. Nová migrácia `apps/api/drizzle/0003_glossy_thaddeus_ross.sql` (`ALTER TYPE ingest_issue_kind ADD VALUE 'empty_guid'`, `ALTER TABLE variant ADD COLUMN guid text NOT NULL`, index `variant_guid_idx`).
- **CRITICAL — regresný integračný test pinuje "surové bajty PRED parsovaním" + "pretrhnuté sťahovanie = rejected, nikdy výnimka"** naraz: skrátený export (odrezaný vnútri zacitovanej bunky prvého záznamu) musí vrátiť `status: "rejected"`, `catalogSnapshots.rawPath` ukazujúci na `.csv.gz`, a nulový počet variantov. Predtým to nemal čo pokryť — vymazanie try/catch alebo presun `storeRawSnapshot` za parsovanie by prešli celou sadou.
- **Important — brána (`judgeSnapshot`) dostáva NOVÉ pole `usableRecordCount`** (`SnapshotCandidate`, `validation.ts`) — počet záznamov, ktoré sa dali naozaj zapísať (`records.length`), nie len `rowCount` (rozparsované riadky). Export, ktorého `code` je prázdny na každom riadku, inak prejde hlavičkovou aj pomerovou kontrolou a blanketový missing-update vymaže celý katalóg (#277/#286 v novej podobe). Podlaha je rovnaká ako `previousRowRatio` (0,8), aplikovaná medzi `usableRecordCount` a `rowCount` NEZÁVISLE od histórie snapshotov.
- **Important — súbežnosť:** `pg_advisory_xact_lock` s JEDNÝM pevným kľúčom (`INGEST_ADVISORY_LOCK_KEY`, nie odvodeným z obsahu) ako prvý príkaz v transakcii serializuje VŠETKY súbežné importy voči databáze — rieši aj súbeh rovnakého obsahu (duplicitný insert), aj súbeh rôzneho obsahu (prekrývajúce sa blanketové missing-updaty). Navyše: SQLSTATE 23505 na `catalog_snapshot_accepted_sha_uq` sa chytá a prekladá na normálny `duplicate` výsledok namiesto uniknutej výnimky.
- **Important — zlyhanie materializácie sa nesmie stratiť:** keď `db.transaction(...)` zlyhá (transakcia sa ROLLBACKNE aj s riadkom `catalog_snapshot`, ktorý mala zapísať), `ingestCatalog` otvorí NOVÝ, samostatný zápis `catalog_snapshot` s `verdict: "rejected"`, tým istým `contentSha256`/`rawPath` a slovenským dôvodom — inak by uložené surové bajty zostali navždy bez záznamu, čo by naň ukazoval. Až potom sa pôvodná výnimka znova vyhodí (`throw`).
- **Minor (6):** `redactSourceLabel` aj v službe (nielen vo fetcheri); zlyhanie parsovania dostáva PEVNÚ slovenskú vetu pre operátora (surová správa sa loguje samostatne, pole `parseError`); variant insert vypisuje polia EXPLICITNE (žiadne `...record`, `supplier` patrí `product`, nie `variant`); `missingCount` zdokumentovaný ako "novo označené", nie "celkovo chýbajúce"; komentár k `INGEST_BATCH_SIZE` opravený na 25 stĺpcov; dopyt na "posledný prijatý snapshot" má sekundárny tie-break `desc(id)`.

Merané na reálnom 54 MB exporte po tejto oprave: **14 014 variantov, 4 519 produktov, 0 anomálií `product_name_conflict`**, gzipnutý surový súbor rádovo jednotky MB.
...
> **Prekonané — pozri CRITICAL dodatok vyššie.** Test nižšie priamo pinuje
> `productKey: "40237"` (starý prefixový kľúč) na riadok `variants`; identita
> produktu je `guid`.
...
> **Prekonané — pozri CRITICAL dodatok pri Task 5 vyššie aj final-wave-a review.**
> Implementácia nižšie predchádza `pg_advisory_xact_lock`-u, prekladu súbehu
> rovnakého obsahu na `duplicate` cez unique-violation, aj dôkazovému
> `rejected` záznamu po zlyhanej materializácii — a stále používa starý
> prefixový `productKey`. Skutočný kód je v `apps/api/src/modules/catalog/ingest.ts`.
...
'
```
Expected (aktualizované review task-5-fix-1 — identita produktu je `guid`, nie prefix `code`, pozri CRITICAL box pri Task 4): `{ status: 'accepted', snapshotId: '…', variantCount: 14014, productCount: 4519, missingCount: 0, issueCount: 0 }`. Uložený `data/catalog-raw/2026/07/*.csv.gz` má rádovo jednotky MB (54 MB CSV sa gzipom stlačí ~10×).
...
- Create: `apps/api/tests/catalog-http-ingest.integration.test.ts` (review
  task-6-fix-1 — testy pre `POST /ingest` vydelené do vlastného súboru, aby
  ani jeden z dvoch nenarástol cez limit 400 riadkov po doplnení review-testov)
...
- Produces:
  ...
  - `getVariant(db: Database, code: string): Promise<VariantDetail | null>` — vracia
    `purchasePrice` vždy; trasa `GET /api/catalog/variants/:code` (nižšie) ho pre
    rolu `citanie` prepíše na `null` pred odoslaním (review task-6-fix-1,
    zámerná odchýlka od pôvodného zadania — nákupná cena je marža)
...
**Review task-6-fix-1 zmenil túto testovaciu sadu oproti hornému bloku** (ten
zostáva ako pôvodný Step 1 zápis, nie ako aktuálny stav súboru). Skutočný
konečný obsah je rozdelený do DVOCH súborov (viď **Files** vyššie) a pridáva:

- `POST /api/catalog/ingest` beží za `requireSameOrigin()` (rovnaká CSRF obrana
  ako F0 na `/api/logout`) — test na cudzí `Origin` a jeden súbežný "busy" test
  (druhé spustenie počas behu prvého vráti `{ status: "busy" }` namiesto
  paralelného importu).
- Audit (`catalog.ingest.trigger`) sa zapisuje AŽ PO dobehnutí importu a nesie
  `data: { status, snapshotId }` — test overuje `actorUserId` aj `data`, nielen
  že akcia bola zapísaná.
- Testy na obe neprivilegované role (`citanie` AJ `sef`) zamietnuté z triggeru,
  a jeden 401 test naprieč všetkými piatimi trasami s FALOŠNÝM aj EXPIROVANÝM
  session tokenom (nielen bez cookie).
- Dva nové ingest-výsledkové testy: druhý import rovnakých bajtov → `duplicate`;
  export, ktorý sa nedá rozparsovať → `rejected` — obe ako normálna 200
  odpoveď, nikdy 500.
- `page` je zhora ohraničené (`max(100_000)`) a prázdna hodnota `page=` sa
  správa ako neprítomná — dva nové testy (`1e21` → 400; `page=` → strana 1).
- Vyhľadávací `q` escapuje `%`/`_` pred vložením do `ILIKE` vzoru — test, že
  dopyt `_`/`%` nevráti celý katalóg.
- Stránkovací test overuje SKUTOČNÝ obsah strán (strana 2 nadväzuje presne na
  koniec strany 1, žiadne prekrytie) namiesto len `total`/dĺžky poľa.
  Snapshot-test seeduje DVA snapshoty s rôznym `fetchedAt` (limit aj poradie sa
  tak dajú overiť skutočne) a `listSnapshots` má sekundárne triedenie podľa
  `id`, aby zhodný `fetchedAt` nemal nedefinované poradie.
- `purchasePrice` sa pre rolu `citanie` vracia ako `null` (zámerná odchýlka od
  zadania — pozri interfaces vyššie); nový test to overuje oboma smermi.
- `productKey`/`productCount` v testoch sa derivujú z databázy pri behu testu,
  nie sú zapísané ako literál — produktová identita sa v paralelnej úlohe
  (Task 4/5 ingest) práve mení z "kód pred lomkou" na `guid`.
...
- [ ] Import reálneho 54 MB exportu prejde a katalóg je v databáze (14 014 variantov, 4 519 produktov — identita produktu je `guid`, review task-5-fix-1) — overené krokom Task 5 / Step 10
