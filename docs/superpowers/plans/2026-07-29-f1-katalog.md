...
**Interfaces:**
- Consumes: `parseShoptetCsv`, `parseDelimited`, `decodeCp1250` (Task 2 — the latter two count `malformedRowCount` for the gate, review task-3-fix-1); `judgeSnapshot`, `DEFAULT_SNAPSHOT_LIMITS`, `SnapshotLimits` (Task 3); `mapRow`, `VariantRecord`, `RowIssue` (Task 4); tabuľky `catalogSnapshots`, `products`, `variants`, `ingestIssues` (Task 1); `Database` (F0)
- Produces:
...
import { DEFAULT_SNAPSHOT_LIMITS, judgeSnapshot, type SnapshotLimits } from "./validation.js";
...
export async function ingestCatalog(
  ...
  const parsed = parseShoptetCsv(download.body);
  const issues: RowIssue[] = [];
  ...
  // Poškodený riadok (napr. jedna nezacitovaná úvodzovka v popise) rozdelí jeden
  // riadok CSV na dva, obidva plné prázdnych polí — `parsed.rows()` to nevidí,
  // lebo `toRecord` (Task 2) ticho dopĺňa/orezáva podľa dĺžky hlavičky. Počíta sa
  // preto SAMOSTATNÝM prechodom nad surovými poľami (`parseDelimited`, Task 2),
  // nie nad hotovými záznamami — druhé rozparsovanie tej istej dekódovanej
  // stránky je cena za to, že sa nemusí meniť tvar `parseShoptetCsv` (review,
  // task-3-fix-1 item 2: parser zostáva čistý, počítanie je na volajúcom).
  ...
  for (const values of parseDelimited(decodeCp1250(download.body))) {
    ...
    if (values.length !== parsed.columns.length) malformedRowCount += 1;
  }
  ...
  // Surové bajty sa ukladajú aj pri odmietnutí — odmietnutý export je dôkaz, nie odpad.
  ...
  return await db.transaction(async (tx): Promise<CatalogIngestResult> => {
    ...
    if (judgement.verdict === "rejected") {
      log.warn(
        { snapshotId, reason: judgement.reason, rowCount, byteSize, columns: parsed.columns.length },
...
- **Brána prijatia (`validation.ts`) je to, čo drží #277, #281 a #286 mimo.** Kontroluje
  v poradí: neprázdnosť a minimálnu veľkosť, prítomnosť povinných STĹPCOV, a počet
  riadkov proti hranici odvodenej z posledného prijatého snapshotu (80 %). Absolútna
  hranica 1 000 riadkov platí LEN pri prvom importe — to bola presne diera #277.
  Nikdy sem nepridávaj „fail-open" vetvu.
