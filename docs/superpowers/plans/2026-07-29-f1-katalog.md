...
export async function ingestCatalog(
  ...
  // dôsledku je rovnaká, akú pridáva `CONSEQUENCE` vo `validation.ts` — #286 platí
  // rovnako pre zlyhané parsovanie ako pre každé iné odmietnutie.
  const judgement: SnapshotJudgement =
    parseErrorMessage !== null
      ? {
          verdict: "rejected",
          reason: `Export sa nedal prečítať: ${parseErrorMessage} Katalóg zostáva nezmenený, import môžete kedykoľvek zopakovať.`,
        }
      : judgeSnapshot(
