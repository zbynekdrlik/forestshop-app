...
describe("decodeCp1250", () => {
  it("dekóduje slovenskú diakritiku zo stredoeurópskej stránky", () => {
    // 0x8d 0x69 0x61 0x70 = "Čiap" v cp1250
    expect(decodeCp1250(Buffer.from([0x8d, 0x69, 0x61, 0x70]))).toBe("Čiap");
  });
});
...
- Produces:
  ...
  - `interface SnapshotCandidate { readonly columns: readonly string[]; readonly rowCount: number; readonly byteSize: number; readonly previousAccepted: { readonly rowCount: number } | null }`
...
function candidate(overrides: Partial<SnapshotCandidate> = {}): SnapshotCandidate {
  return {
    ...
    previousAccepted: overrides.previousAccepted ?? { rowCount: 14_014 },
  };
}
...
**Interfaces:**
- Consumes: `parseShoptetCsv` (Task 2); `judgeSnapshot`, `DEFAULT_SNAPSHOT_LIMITS`, `SnapshotLimits` (Task 3); `mapRow`, `VariantRecord`, `RowIssue` (Task 4); tabuľky `catalogSnapshots`, `products`, `variants`, `ingestIssues` (Task 1); `Database` (F0)
- Produces:
...
export async function ingestCatalog(
  ...
  const judgement = judgeSnapshot(
    { columns: parsed.columns, rowCount, byteSize, previousAccepted: previous ?? null },
