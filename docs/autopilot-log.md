...
- Verzia: 0.3.0-dev.255 (prvý commit; pri integrácii prečíslované na
  0.3.0-dev.256 — sibling issue 425 medzitým obsadil .255).
...
## issues 439 + 440 — kalendár 3 dni + emoji picker (batch, 1 PR)
- PR #441 → main merge `ab0b92f` ; nasadené v0.3.0-dev.262 na forestshop.newlevel.media (verzia z DOM overená, konzola 0 chýb/0 varovaní).
- **439:** `resolveNextEvents` (`modules/calendar/next-event.ts`) zoskupuje po kalendárnom dni (`zonedDateKey`, Europe/Bratislava) a oreže na `NEXT_EVENT_DAYS_LIMIT=3` DNÍ (nie udalostí); dni bez udalosti sa preskočia, `continue` nie `break`. Konštanta `NEXT_EVENTS_LIMIT`→`NEXT_EVENT_DAYS_LIMIT`, param `limit`→`dayLimit`. Návratový tvar/route/schéma/karta bez zmeny (ploché pole, formát riadku `📅 deň — názov`). Živo: 18.8. (3 udalosti) + 19.8. (1) + 26.2. budúci rok (celodenná, ~6 mes. medzera preskočená). Testy `next-event.test.ts` (day-grouping rozlišuje od event-slicingu: dayLimit 1 → všetky udalosti 1 dňa).
- **440:** emoji sa ukladajú/zobrazujú SPRÁVNE (žiadny bug — overené naživo, zamknuté `emoji-persist.integration.test.ts` POST→GET). Chýbal len VSTUP. Nový `EmojiPickerButton.tsx` (+ čistý `insertEmojiAtSelection` helper, kurátorovaná sada, žiadna externá knižnica); vkladá na kurzor, popover sa po vložení ZAVRIE (inak prekryje Uložiť — nájdené e2e), `role="group"` nie `menu`. Zapojené: Poznámky textarea + Nadpis/Podrobnosti Nového upozornenia. Testy: unit (`EmojiPickerButton.test.tsx` 8), integračná perzistencia (2), e2e (`poznamky.spec.ts` desktop+375px, `upozornenia.spec.ts`).
...
## 2026-08-19 — batch issues 446 + 447 + 448 (PR #454, merge 39dd0159, v0.3.0-dev.267 na dev)
- version bump 0.3.0-dev.266 → dev.267 (po merge PR #454)
- #446 badge Párovanie = aktívne nenapárované: RED apps/api/tests/pairing-review-catalog-coverage-http.integration.test.ts "activeUnpaired" (feec1be) → GREEN queries.ts computeCatalogCoverage vracia activeUnpaired (aktívny ≥1 sellable, bez efektívneho odkazu, bez terminálneho rozhodnutia) + pairingReviewApi.ts badge číta activeUnpaired namiesto total (9670f27). PROD: badge 2302 → 449 (SELECT: catalog_active 2530, catalog_linked 2081, activeUnpaired 449).
- #448 supplier-stock zber efektívne odkazy: RED apps/api/tests/supplier-stock-override-links.integration.test.ts (bb86b49) → GREEN run.ts collectSupplierLinks volá resolveEffectiveSupplierLink (override ∪ internalNote), odstránený isNotNull(internalNote) filter (79819f2). .claude/rules/supplier-stock.md zdroje populácie. PROD: 17 override riadkov, 1 override-only (huntingshop.eu) teraz vstupuje do zberu.
- #447 pilulka "Beží": automationStatus.ts buildAutomationStatus + STATIC_AUTOMATION_STATUS, App.tsx fetchRestockStatus, e2e nav.spec.ts (8cb0955). E2E regresia (pilulka zanášala prístupný názov tab tlačidla → restock exact:true click timeout): aria-hidden na pilulke (f00c54b) — lokálne 5/5 e2e zelené. PROD: pilulky "Beží" na Vypredané → Skladom / Sync / Dodávateľský sklad.
- CI f00c54b: všetko zelené; main deploy 39dd015 success; naživo overené (badge 449, pilulky, konzola 0/0, v0.3.0-dev.266).

## 2026-08-19 — integrácia batch issues 450 + 452 (PR #456, merge 8f453134, v0.3.0-dev.267 na main)
- Serial integračný merge worktree vetvy (lane pripravil obe úlohy, lokálne zelené, review 0 🔴/🟡/🔵). Merge `e69e195` do dev, `pnpm gates:local` zelené (700 unit), PR #456 dev→main, CI všetko zelené, merge `8f453134`, Deploy success.
- **#452:** dodávateľské čipy na „Na objednanie" zoradené abecedne (sk `localeCompare`) cez `sortSuppliersForChips` (`ordersSummary.ts`), čip „Všetci" prvý (`OrdersToolbar.tsx`). Živo: poradie Všetci, BETALOV, Drevo Novák, FOMEI SLOVAKIA, Forest, GRUBE, … WETLAND, ZUBÍČEK.
- **#450:** nová položka menu „Riešiť" 🤔 v Eshope hneď POD „Nedostupné tovary" (`nav.ts`, bez `wide`), placeholder obrazovka `RiesitSection.tsx` (`data-testid=riesit-placeholder`). Zadanie Štěpán (Discord 19.8.2026), funkcia sa upresní neskôr.
- **nav.spec.ts merge:** obe strany menili e2e nav.spec.ts — dev pridal issue-447 stavové pilulky (bez viditeľnej záložky), worktree počty 20→21 za „Riešiť". Git ort auto-merge čistý; súčet záložiek 21 správny (dev nepridal viditeľnú záložku).
- Živo overené na forestshop.newlevel.media (v DOM `v0.3.0-dev.267`, konzola 0 chýb/0 varovaní). Tickety zavreté ručne až po živom overení (mali post-deploy akceptačnú podmienku, telo PR ich zámerne nezatváralo).

## 2026-08-19 — integrácia issue 453 (PR #457, merge 8c4f012, v0.3.0-dev.268 na main)
- Serial integračný merge worktree vetvy (lane pripravil #453, lokálne zelené incl. izolované PG integračné + e2e, review 0 🔴/🟡/🔵). Merge `de2460f` do dev, `pnpm gates:local` zelené (api 998 + web 702 unit), PR #457 dev→main, migrácia `0057_amused_thundra` (stĺpec `quantity` na `floor_note_product`).
- **#453:** počet kusov pri pripnutom produkte v Objednávkach predajne — `floor_note_product.quantity` (int NOT NULL DEFAULT 1, CHECK `>= 1`), vstup počtu pri pripínaní + inline spinbutton v zozname (`FloorNoteProductChip.tsx`), render „N ks", API validácia `>= 1`.
- **CI contention:** push beh (`32265668710`) aj pull_request beh (`32265697646`) nad tým istým SHA `de2460f` bežali súčasne; pull_request `e2e` visel ~20 min (contention na runneri), kým push `e2e` prešiel za ~1 min. Zrušený + `e2e` job spustený znova sám → OK za ~2 min. Trvalá oprava: issue 458. Gotcha zapísaná do `.claude/rules/ci.md`.
- Živo overené na forestshop.newlevel.media (v DOM `v0.3.0-dev.268`): testovací zápis → pripnutý produkt s počtom 2 → „2 ks", úprava na 3 + reload → „3 ks" pretrvalo, existujúci zápis „1 ks", konzola 0 chýb/0 varovaní, testovací zápis zmazaný. Ticket #453 zavretý ručne po živom overení (telo PR ho zámerne nezatváralo).

## 2026-08-19 — integrácia issues 455 + 458 (PR #459, merge 52daba75, v0.3.0-dev.269 na main)
- Serial integračný merge lane vetvy `worktree-agent-a967e7a3b0acdca69` (#455 el.value fix) + implementácia #458 na dev. PR #459 dev→main, CI zelené, merge `52daba75`, Deploy success.
- **#455 (emoji flake, DVE vrstvy):** (1) `el.value`: insert číta bázu zo živého DOM-u, nie zo zaostávajúceho propu (lane RED `2e5fd89` → fix `1643c92`). (2) HLBŠIA príčina (toto kolo): oneskorený `requestAnimationFrame` na obnovu fokusu UKRADOL fokus inému poľu, keď na zaťaženom CI vystrelil počas `.fill()` druhého poľa (dvojkroková focus→insertText) → text do zlého poľa, podrobnosti prázdne → holé "✅". Guard: rAF obnoví fokus len keď je `document.activeElement` stále náš (pole/root/body/null), rAF id v refe + `cancelAnimationFrame` pri inserte aj unmounte. Diagnóza Fable hard-debug consult. RED `6657020` → GREEN `e6d0586` (deterministický unit test fokus-steal). e2e bariéry `expect(pole).toBeFocused()`. `frontend-design.md` doplnené.
- **#458 (CI dvojbeh):** `pull_request` trigger ODSTRÁNENÝ z `ci.yml` (CI len na `push` do dev aj main). Prvý pokus (concurrency-cancel push+PR, `2c2beea`) zavrhnutý — zrušený beh nechal CANCELLED check na head SHA → PR trvalo UNSTABLE (overené na `e6d0586`). Push beh reportuje checky na PR podľa SHA → jeden beh, PR CLEAN. Dôkaz: PR head `2761a26` = 1 beh (`32282959261`) → CLEAN; main `52daba7` = 1 CI (`32284317402`) + 1 Deploy (`32284317560`), žiadny push+PR pár. `ci.md` aktualizované.
- **Transient:** `playwright install --with-deps chromium` ojedinele zamrzol (~19 min, spoločný CDN výpadok) na PR behu `e6d0586`; `gh run cancel` + `gh run rerun` (`32279109468` attempt 2) prešiel. Zdokumentované v `ci.md`.
- Živo overené na forestshop.newlevel.media (v DOM `v0.3.0-dev.269`, commit `52daba75`): Upozornenia → Nové upozornenie → nadpis `Test overenie 🔥` + podrobnosti `Skontrolovat sklad ✅` (text zachovaný + emoji na OBOCH poliach), konzola 0 chýb/0 varovaní. Tickety #455/#458 zavreté ručne po živom overení (telo PR ich zámerne nezatváralo). Cleanup: lane vetva + worktree + `refs/autopilot-wip/*` zmazané.

## 2026-08-19 — integrácia issues 451 + 460 (PR #461, merge 3f19ce3, v0.3.0-dev.270 na main)
- Serial integračný merge lane vetvy `worktree-agent-a011486b2a152d2c6` (#451, HEAD `e013d75`, 4 commity) + implementácia #460 na dev. Merge `1ee0367` do dev bol čistý (dev sa od bázy `8c4f012` na dotknutých súboroch nerozišiel, #445 zmeny už v báze). `pnpm gates:local` zelené. PR #461 dev→main, CI zelené (`32292040322`), merge `3f19ce3`, Deploy success. Žiadny `Closes` v tele PR (odložené naživo overenie) — tickety zavreté ručne.
- **#451:** DPD denný zvoz je PRIMÁRNA akcia záložky. „Objednať zvoz na deň" = `btn good lg`, PRVÉ v DOM (nad per-zásielkovým `btn good` „Objednať prepravu DPD" aj nad tabuľkou); feedback `<p role="status" data-testid="dpd-pickup-message">`. Robot zatvára prekrývajúci DPD info banner (`portal-fill.ts dismissInfoBanners`, fail-soft, RED `ee055fa`→GREEN `eb64115`). Kontakt (Štěpánov krok 5) + presný banner selektor ostávajú UNVERIFIED do prvého reálneho zvozu (dpd.md).
- **#460 (CI Chromium odolnosť):** (1) Playwright browser cache (`~/.cache/ms-playwright`, `actions/cache@v4`, kľúč `Linux-playwright-1.62.0`) v `integration` aj `e2e` — na cache-HIT len `install-deps` (apt), na MISS plná `--with-deps`. Dôkaz z merge behu: `Cache not found` → plná inštalácia → `install-deps (cache hit)` SKIPPED → `Post Cache success` → `Cache saved with key: Linux-playwright-1.62.0`. Paralelný zápis kľúča neškodný. (2) `TEST_TIMEOUT_MS` 60→120 s v 6 real-Chromium integračných testoch (~16 s baseline, ~8× rezerva); globálny `vitest.config.ts` 30 s nezmenený. Job bezpodmienečný (issue 351) — `if:` len na kroku. `ci.md` aktualizované.
- Živo overené na forestshop.newlevel.media/?tab=dpd (v DOM `v0.3.0-dev.270`, Playwright DOM-only, reálny zvoz NEobjednaný): zvoz `btn good lg` prvý (`zvozBeforePerShipment: true`), per-zásielkové `btn good` druhé, tabuľka tretia (`perShipmentBeforeTable: true`), badge „28" nezmenený, konzola 0 chýb/0 varovaní. Cleanup: lane vetva + worktree + `refs/autopilot-wip/worktree-agent-a011486b2a152d2c6` zmazané.

## 2026-08-20 — issue 462 (PR #463, merge e80549b, v0.3.0-dev.271 na main)
- **#462 (CI install-deps na cache-HIT):** `playwright install-deps` (apt) odstránený z cache-HIT vetvy v oboch joboch (`integration`, `e2e`) — runner je čerstvá `ubuntu-latest` VM a cache obnoví len binárku prehliadača, systémové liby dáva image, takže apt zmizol z hot path a jeho ~17-min freeze (apt zámok/network na zdieľanom runneri) sa eliminoval. `install --with-deps` beží už len na cache MISS a má `timeout-minutes: 10` poistku (transient hang zlyhá automaticky červeno pri 10 min, žiadny continue-on-error). Joby ostávajú bezpodmienečné (issue 351), `if:` len na MISS kroku.
- **Živý dôkaz:** behy `32317258896` + `32318247998` (dev) a `32318913324` (main merge) — v OBOCH joboch cache HIT, install krok `skipped`, reálny Chromium (`test:integration`/`e2e`) prešiel bez apt → potvrdené, že `ubuntu-latest` má chromium liby aj bez `install-deps`. actionlint (1.7.12) na `ci.yml` = clean.
- **Recenzia** (čerstvý general-purpose, Opus 4.8): 0 R 1 Y 3 B. Y (ci.md hlásilo starý stav) vyriešené v tej istej vetve (`d74df36`). Follow-up #464 (Playwright Docker image ako plne deterministická alternatíva, cross-cutting).
- PR #463 dev->main, 5/5 brán zelených, merge `e80549b`, Deploy success (`32318913300`). Živo overené: DOM forestshop.newlevel.media = `v0.3.0-dev.271`. #462 zavreté ručne po overení (telo PR = plain „issue 462", žiadny Closes). `ci.md` #460/#462 sekcia aktualizovaná.

## 2026-08-20 — issue 464 (evaluate-and-decide → ZAVRETÉ ako rozhodnuté, bez implementácie)
...
- issue 561 follow-up (v0.3.0-dev.339): Nález 2 — `buildRestockAfterSupplierStock` posiela do `startRunNow` vlastný `new Date()` (nie tick-ové `now` supplier-stocku), aby `job_run.started_at` reťazeného restocku niesol jeho skutočný čas štartu (~04:17), nie štart supplier-stocku (02:20). RED c18f67e → GREEN df433e9 (restock-chained.integration.test.ts, started_at > tick now). Playbook: supplier-stock.md (Nález 1 elapsedMs artefakt www/non-www miešania + meranie 21. 9.: 117 min, wetland 1594/1282/163, tthunt 335/240/25, predfilter ~10 %), scheduler.md (job B posiela vlastný new Date()). Bump commit 61333ae.
- issue 575 (v0.3.0-dev.340): predajňový (floor) riadok v „Na objednanie" dostal rovnaké ovládanie ako e-shopová objednávka — kód s odkazom pod 🛍️, 🔗 odkaz na dodávateľa (+ ✏️ cez zdieľaný `POST /api/product-links/:productKey`), stavové tlačidlá (`floor_note_product.state`, REUSE enumu `order_line_state`, migrácia 0065) a per-položková poznámka (`floor_note_product.comment`, viditeľná na čipe v Objednávky predajňa). API: `POST .../products/:variantCode/state` + `PATCH .../comment` (tx+audit, 404, role). „Riešiť" zahŕňa floor riadky so `state=riesit` (samostatná „Predajňa" tabuľka v `RiesitSection`), `countOpenOrdersByState` ich ráta. Zdieľané primitívy: `StateButtons` (idKey), `OrderSupplierLinkDisplay` (data prop), `useFloorRowMutations` + `floor-order-rows.ts` (vyňaté kvôli max-lines). Testy: unit ordersSummary/floorRows, integ floor-notes-products-state (6) + floor-orders-http rozšírené, e2e floor-orders-board (kód odkaz+poznámka+stav Riešiť). gates:local EXIT=0; integ (targeted) + e2e (floor-orders-board+orders-layout) zelené. Worktree worker; PR/merge/deploy rieši supervisor. Commity 773ab75 (bump) → 9adf5d0 → 6667117 → 3bba304.
- issue 577 (v0.3.0-dev.341): popis stavu riadku `objednane` premenovaný „Nevybavené" → „Nemáme" (Štěpán, Discord Develop-ÚLOHY) v jedinom zdroji `orderLineStateLabels.ts` — všetci konzumenti (`OrderLineStateButtons`/`StateButtons`, `OrderLineRow`, `FloorOrderRow`, `FloorNoteProductChip`) renderujú label odtiaľ, žiadny duplicitný `STATE_LABELS` pre order-line stav neexistuje (ostatné mapy sú pre product/variant stavy). DB enum hodnota `objednane` NEZMENENÁ (žiadna migrácia). Shoptet stav objednávky „Nevybavená" (`stuck-status.ts` + testy) je INÁ vec — ostáva; stale-order tooltip „Nevybavená objednávka stará N dní" (OrderLineRow) tiež (iný koncept). RED 2cc0f1f (unit OrderLineStateButtons/OrdersSection + e2e orders/orders-objednane-state assertion-y → „Nemáme") → GREEN abe64a2. Playbook: orders.md (label „Nemáme"), autopilot-log. gates:local EXIT=0. Worktree worker; PR/merge/deploy rieši supervisor. Commity 55ef2cc (bump) → 2cc0f1f → abe64a2.
- issue 579 (v0.3.0-dev.342): východiskový stav riadku = ŽIADNY + „Nemáme" červené (Štěpán, Discord Develop-ÚLOHY). `order_line.state` a `floor_note_product.state` sú NULLABLE bez DEFAULT (migrácia 0066: `DROP NOT NULL`/`DROP DEFAULT` + jednorazový `UPDATE … SET state = NULL WHERE state = 'objednane'` reset existujúcich riadkov — Štěpán chcel začať označovať odznova; enum nezmenený, žiadna 55P04 pasca). Nový riadok (ingest/floor attach) sa rodí bez stavu (NULL) — kód `state` pri inserte neuvádza, netreba zmenu. Typy: `OrderLine["state"]` = `OrderLineStateValue | null` (web) / `OrderLineState | null` (API interface); settery/POST body ostávajú NON-NULL enum (nový `OrderLineStateValue` typ, „odznačiť" mimo zadania). Sémantika: `isLineResolved`/`isFloorRowResolved` = `ordered || (state !== null && state !== "objednane")` (NULL aj „Nemáme" = nevybavené). UI: NULL → žiadne aktívne tlačidlo (`StateButtons active = state === s`), read-only bunka „—", `FloorNoteProductChip` ukáže odznak keď `state !== null` (vrátane „Nemáme" červený). CSS: `.ord-state-btn-objednane` (popis vždy červený) + `.active` (výplň) + `.pill.objednane` = `--fs-danger`/`--fs-danger-bg` (žiadny nový raw hex). RED e2dc18f (web typy nullable + testy: ordersSummary NULL, OrderLineStateButtons NULL, FloorNoteProductChip červený „Nemáme") → GREEN (schéma+0066, API typy, sémantika, chip, CSS, integračné order-state-reset (migračný UPDATE reset z .sql, POST na NULL), aktualizované fixtúry orders-ingest/orders-http/orders-http-state/floor-orders-http/floor-notes-products-http/order-schema/orders-state-lock/e2e-setup, e2e orders-objednane-state NULL+červené+reload). Playbook: orders.md (NULL východiskový stav, reset 0066, sémantika vybavené, „Nemáme" červené), frontend-design.md (červená „Nemáme" cez --fs-danger, CSS špecificita). gates:local EXIT=0. Worktree worker; PR/merge/deploy rieši supervisor.
- issue 579 (CI-fix, PR #580): dva zastarané testy zosúladené s NULL-východiskovým stavom — `floor-notes-products-state-http.integration.test.ts` (audit `from: "objednane"` na `from: null`, nová položka sa rodí bez stavu) + `orders-overview.spec.ts` (in-test `isResolved` kópia + typ `RawOrderLine.state` na kanonický `ordered || (state !== null && state !== "objednane")` — NULL fixtúry appka ráta do „Položiek na objednanie" 9, test čakal 0). Produkčný kód správny, len testy zaostali. gates:local EXIT=0; cielený integ (6/6) + e2e orders-overview zelené lokálne (5433). Playbook: testing.md (e2e kópia predikátu tíško zastará pri zmene kontraktu). Worktree worker; PR/merge rieši supervisor. Commit b6a7052.
- issue 581 (v0.3.0-dev.343): spresnenie 579 — neaktívne „Nemáme" (stav `objednane`) je sivé ako ostatných 5 stavových tlačidiel, červené LEN po kliknutí (Štěpán, Discord Develop-ÚLOHY: „východiskový stav kariet musí byť všade farebne rovnaký — písmo sivé; keď kliknem na Nemáme, bude červená"). Zmazané issue-579 CSS pravidlo `.ord-state-btn-objednane { color: var(--fs-danger) }` (popis vždy červený) — neaktívne „Nemáme" padá na základnú `.ord-state-btn { color: var(--fs-ink-muted) }`. Ponechané: `.ord-state-btn-objednane.active` (červená výplň/rám/text) + `.pill.objednane` (read-only odznak len pri zvolenom stave). RED 30c4967 (e2e orders-objednane-state: neaktívne „Nemáme" `color` = neaktívne „Skladom" `color`, po kliknutí = `--fs-danger`) → GREEN a00e37f. Playbook: frontend-design.md (červená len `.active`). gates:local EXIT=0. Worktree worker; PR/merge/deploy rieši supervisor.
- issue 583 (v0.3.0-dev.344): počítadlá „Výmena tovaru"/„Vrátený tovar" v ľavom menu červené (`--fs-danger`, tá istá červená ako aktívne Nedostupné/Nemáme v „Na objednanie") s bielym číslom, LEN tieto dve (Štěpán, Discord Develop-ÚLOHY 22. 9.). `NavTab.badgeTone?: "danger"` v registri `nav.ts` (princíp wide/icon/defaultCollapsed, issue 343) → `Sidebar.tsx` generický modifikátor `tab-badge-danger` podľa `tab.badgeTone` (testid/aria-label nezmenené) → `app.css` `.tab-badge.tab-badge-danger { background: var(--fs-danger) }` (biele číslo dedí z `.tab-badge`, žiadny nový raw hex); súhrnný `folder-badge-*` zbaleného priečinka ostáva zelený. RED d7d462a (nav.test: badgeTone presne exchange/returned; Sidebar.test: trieda len pri badgeTone + súhrnný odznak holý `tab-badge`; e2e order-flags: `nav-badge-exchange`/`returned` background = `--fs-danger` sonda + color biela, `nav-badge-orders` ostáva `--fs-brand`) → GREEN f297f09; e2e RED dokázaný aj naživo (dočasné prepnutie tokenu na `--fs-brand` → 1 failed, obnovené). Playbook: frontend-design.md (badgeTone v registri + `.tab-badge-danger`, sonda tokenu v e2e). gates:local EXIT=0 (api 1145, web 818); e2e order-flags 1/1 lokálne (5433, chromium 1234 pre playwright 1.62). Worktree worker; PR/merge/deploy rieši supervisor.
