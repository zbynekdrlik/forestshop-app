...
test("manažér vidí stav katalógu, vyhľadá variant a konzola je čistá", async ({ page }) => {
  ...
  // + 2 seedované produkty pre "Objednávky predajňa" ("E2E-PREDAJNA-1"/
  // "E2E-PREDAJNA-2", issue 410, `scripts/e2e-fixtures-floor-notes.ts`) = 105.
  // Prví dvaja (PREP-1/PREP-2) sú `out_of_stock` (nemenia "sellable"/
  // "missing" nižšie), zvyšných 5, všetkých 60 "E2E-PAGE-*", všetky 3
  // "E2E-PR-*" a oba "E2E-PREDAJNA-*" sú `sellable` (posúvajú filter
  // "sellable" 7→75 nižšie), "missing"(1) sa nemení ani jedným z nich.
...
test("filter podľa stavu zúži zoznam na predajné varianty", async ({ page }) => {
  ...
  // "E2E-PR-NENAJDENY"/"E2E-PR-SLINKOU") + 2 sellable produkty issue 410's
  // fixtúry ("E2E-PREDAJNA-1"/"E2E-PREDAJNA-2"). (issue 311's 2 sellable
  // "E2E-RL-NAVRH"/"E2E-RL-CUDZI" vypadli — issue 387 E8 fixtúru odstránilo,
  // vtedajší súčet bol 75, teraz zhodou okolností opäť 75 po pridaní issue
  // 410's dvoch nových.)
