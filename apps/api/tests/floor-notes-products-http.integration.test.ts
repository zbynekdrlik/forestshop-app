...
describe("POST /api/floor-notes/:id/products", () => {
  it("pripne existujúci produkt, GET zoznamu ho vráti VRÁTANE priamej shop_product_url adresy", async () => {
    ...
    // issue 575/579: položka nesie aj `state` (východiskovo NULL = neoznačený) + `comment` (null).
    expect(row?.products).toEqual([{ variantCode: "E2E-PIN-1", productName: "Test produkt E2E-PIN-1", sizeLabel: null, quantity: 1, shopUrl: "https://www.forestshop.sk/pin-1/", state: null, comment: null }]);
  });
